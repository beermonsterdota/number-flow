...
<Match vanilla>
<Note>All custom attributes can also be set as camelCased properties on the <Comp/> element.</Note>
</Match>
...
<Match vanilla>
```html
<number-flow format="{ notation: 'compact' }"></number-flow>
```
</Match>
...
<Match vanilla>
```html
<number-flow locales="en-US"></number-flow>
```
</Match>
...
<h3>
	<code>trend<Type>: <Union types={['number', '(prev: number, value: number) => number']} /></Type></code>
	<Meta>Default: `(prev, value) => value - prev`</Meta>
</h3>

Controls the direction of the digits.

- `+number:` The digits always go up.
- `0:` No overall trend; each digit goes up if it increases and down if it decreases. This can be useful if you
want to animate updates without conveying an overall increase or decrease ([example](https://x.com/pontusab/status/1825941664189526067)).
- `-number:` The digits always go down.
